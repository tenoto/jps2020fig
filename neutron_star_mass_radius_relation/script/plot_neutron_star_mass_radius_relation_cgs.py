...
df_rhoP = pd.read_excel('data/EOS_v210223.xlsx',engine='openpyxl',
	...
	names=['density','n/a','pressure','n/a','energy_density','n/a','pressure_cgs','density_cgs'])
...
Raai_core_x = 10**df_Raai['core-X'].dropna()
...
#       = 5.609e-13 MeV fm-3
Raai_core_y = 10**df_Raai['core-Y'].dropna()
...
Raai_cont_x = 10**df_Raai_cont['X'].dropna()
Raai_cont_y = 10**df_Raai_cont['Y'].dropna()
...
for sheet_name in mr_model_list:
	print(sheet_name)
	ax1.plot(df_rhoP[sheet_name]['density_cgs'],df_rhoP[sheet_name]['pressure_cgs'],
...
#ax1.axvline(x=2.75e+14,color="#F39C12",zorder=zorder);zorder+=1
#ax1.set_xlim(0.01,0.18)
#ax1.set_ylim(0.01,2.0)
ax1.set_xlim(1.5e+14,1.6e+15)
ax1.set_ylim(1e+33,1e+36)
ax1.set_xlabel('Total energy density (g cm$^{-3}$)')
ax1.set_ylabel('Pressure (dyn cm$^{-2}$)')
ax1.legend(loc='lower right',borderaxespad=1,fontsize=10,ncol=2)
